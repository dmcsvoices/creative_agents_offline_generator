...
parser = argparse.ArgumentParser(
    description="Z-Image Turbo - Feb15 Update - A converted ComfyUI workflow"
)
parser.add_argument(
    "--text7",
    default="A beautiful landscape",
    help='Image prompt text (main prompt argument)',
)
...
PROMPT_DATA = json.loads(
    '{"9": {"inputs": {"filename_prefix": "z-image", "images": ["43", 0]}, "class_type": "SaveImage"}, "39": {"inputs": {"clip_name": "qwen_3_4b.safetensors", "type": "lumina2", "device": "default"}, "class_type": "CLIPLoader"}, "40": {"inputs": {"vae_name": "ae.safetensors"}, "class_type": "VAELoader"}, "41": {"inputs": {"width": 1024, "height": 1024, "batch_size": 1}, "class_type": "EmptySD3LatentImage"}, "42": {"inputs": {"conditioning": ["45", 0]}, "class_type": "ConditioningZeroOut"}, "43": {"inputs": {"samples": ["44", 0], "vae": ["40", 0]}, "class_type": "VAEDecode"}, "44": {"inputs": {"seed": 397889736320838, "steps": 9, "cfg": 1, "sampler_name": "res_multistep", "scheduler": "simple", "denoise": 1, "model": ["47", 0], "positive": ["45", 0], "negative": ["42", 0], "latent_image": ["41", 0]}, "class_type": "KSampler"}, "45": {"inputs": {"text": "", "clip": ["39", 0]}, "class_type": "CLIPTextEncode"}, "46": {"inputs": {"unet_name": "z_image_turbo_bf16.safetensors", "weight_dtype": "default"}, "class_type": "UNETLoader"}, "47": {"inputs": {"shift": 3, "model": ["46", 0]}, "class_type": "ModelSamplingAuraFlow"}}'
)
...
def main(*func_args, **func_kwargs):
    ...
    with torch.inference_mode(), ctx:
        ...
        for q in range(args.queue_size):
            # Use command-line argument for prompt text
            ...
            seed = parse_arg(args.seed11) if args.seed11 is not None else random.randint(1, 2**64)
